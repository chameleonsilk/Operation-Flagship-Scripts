...
for i = 1, 1000 do
mudPsn = mist.getRandPointInCircle(mzone.point, mzone.radius * 0.99, mzone.radius * 0.25)
	...
	break
		...
		--local smoke_marker = mist.utils.zoneToVec3(mzone)
		
		
			
--timer.scheduleFunction(Make_Smoke, smoke_marker, timer.getTime() + 5)
			
return
end
---
------------------------------------------------------------------------------------------------------------------------------------------------------------------
function Make_Smoke(_smoke, time)
------------------------------------------------------------------------------------------------------------------------------------------------------------------
local smoke = _smoke

if Smoke == 1 then
trigger.action.smoke({x= smoke.x + math.random(1,200), y= land.getHeight({x = smoke.x, y = smoke.z}), z= smoke.z + math.random(1, 200)}, trigger.smokeColor.red)
return time + 90
end
